...
export default class HTML5Visu {
  ...
  async showMainVisu() {
    ...
    ReactDOM.render(
      <React.StrictMode>
        <Spinner
          text={"Preload visualisations on first pageview ..."}
        ></Spinner>
      </React.StrictMode>,
    ...
    const App = observer(() => {
      return (
        <div
          style={{
            width: stateManager!.get("VISUWIDTH")!.toLowerCase(),
            height: stateManager!.get("VISUHEIGHT")!.toLowerCase(),
            userSelect: "none",
          }}
        >
          {stateManager.get("ISONLINE") === "TRUE" ? (
            <Visualisation
              visuname={stateManager!.get("CURRENTVISU")!.toLowerCase()}
              mainVisu={true}
              replacementSet={null}
  ...
  pathConfiguration(): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      fetch(this.rootDir + "/visu_ini.xml").then((response) => {
        // Path is correct
        if (response.ok) {
          resolve(true);
        } else {
          fetch(this.rootDir + "/plc/visu_ini.xml").then((response) => {
            // Path is correct
            if (response.ok) {
              ...
              fetch(this.rootDir + "/webvisu/visu_ini.xml").then((response) => {
                // Path is correct
                if (response.ok) {
      ...
      fetch(url, {
        ...
        if (response.ok) {
          response.arrayBuffer().then(async (buffer) => {
            ...
            // Get the download ID
            let xmlDownloadID = data.getElementsByTagName("download-id")[0]
              .textContent;
            ...
            let xmlCompression = data.getElementsByTagName("compression")[0]
              .textContent;
            ...
            let xmlBestFit = data.getElementsByTagName("best-fit")[0]
              .textContent;
            if (xmlBestFit === "true") {
  ...
  async preloadVisus() {
    ...
    let visusToBeLoaded = [
      StateManager.singleton().oState.get("STARTVISU").toLowerCase(),
    ];
