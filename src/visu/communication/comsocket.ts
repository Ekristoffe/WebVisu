          ...
          switch (stack[position][0]) {
            case 'var':
              if (ComSocket.singleton().oVisuVariables.has(value)) {
                const varContent = ComSocket.singleton().oVisuVariables.get(value)!.value
        ...
        .then((response) => {
          response.arrayBuffer()
            .then((buffer : ArrayBuffer) => {
              ...
              const transferarray : Array<string> = (text.slice(1, text.length - 1).split('|'))
              if (transferarray.length === this.requestFrame.listings) {
                for (let i = 0; i < transferarray.length; i++) {
                  const varName = this.lutKeyVariable[i]
