...
export function createGroupObject(
    ...
    const relCoord = {
        width: groupShape.rect[2] - groupShape.rect[0],
        height: groupShape.rect[3] - groupShape.rect[1],
    };
    // the relCenterCoord are the coordinates of the midpoint of the div
    const relMidpointCoord = {
        x: (groupShape.rect[2] - groupShape.rect[0]) / 2,
        y: (groupShape.rect[3] - groupShape.rect[1]) / 2,
    };
    ...
    const edge =
        groupShape.lineWidth === 0 ? 1 : groupShape.lineWidth;
    // Compute the strokeWidth through hasFrameColor
    const lineWidth = groupShape.hasFrameColor ? edge : 0;
    // Compute the fill color through has_fill_color
    const fillColor = groupShape.hasInsideColor
    ...
    const initial: IGroupObject = {
        ...
        normalFillColor: groupShape.fillColor,
        alarmFillColor: groupShape.fillColorAlarm,
        normalFrameColor: groupShape.frameColor,
        alarmFrameColor: groupShape.frameColorAlarm,
    ...
    Object.defineProperty(initial, 'writeAccess', {
        get: function () {
            ...
            if (!isNaN(currentNum)) {
                if (
                    groupShape.accessLevels[currentNum].includes(
                        'w',
                    )
                ) {
                    return true;
    ...
    Object.defineProperty(initial, 'groupScale', {
        get: function () {
            const xscaleFactor =
                relCoord.width / groupShape.groupSize[0];
            const yscaleFactor =
                relCoord.height / groupShape.groupSize[1];
