      ...
      get: function () {
        ...
        if (interval !== 0) {
          if (initial.lowerBound < initial.upperBound) {
            let interim =
              ((initial.relCoord.width - 2 * initial.b1 - initial.b2) *
                Math.abs(initial.value - initial.lowerBound)) /
              interval;
            return interim;
          } else {
            let interim =
              (initial.relCoord.width - 2 * initial.b1 - initial.b2) *
              (1 - Math.abs(initial.value - initial.upperBound) / interval);
            return interim;
          }
        } else {
          return 0;
    ...
    Object.defineProperty(initial, "scrollvalue", {
      get: function () {
        ...
        if (interval !== 0) {
          if (initial.lowerBound < initial.upperBound) {
            let interim =
              ((initial.relCoord.height - 2 * initial.b1 - initial.b2) *
                Math.abs(initial.value - initial.lowerBound)) /
              interval;
            return interim;
          } else {
