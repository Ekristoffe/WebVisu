...
export function createPiechartObject (piechartShape: IPiechartShape, dynamicElements : Map<string, string[][]>) : IPiechartObject {
  ...
  const absCornerCoord = { x1: piechartShape.rect[0], y1: piechartShape.rect[1], x2: piechartShape.rect[2], y2: piechartShape.rect[3] }
  // absCenterCoord are the coordinates of the rotation and scale center
  const absCenterCoord = { x: piechartShape.center[0], y: piechartShape.center[1] }
  // relCoord are the width and the height in relation the div
  const relCoord = { width: piechartShape.rect[2] - piechartShape.rect[0], height: piechartShape.rect[3] - piechartShape.rect[1] }
  // the relCenterCoord are the coordinates of the midpoint of the div
  const relMidpointCoord = { x: (piechartShape.rect[2] - piechartShape.rect[0]) / 2, y: (piechartShape.rect[3] - piechartShape.rect[1]) / 2 }
  ...
  const edge = (piechartShape.line_width === 0) ? 1 : piechartShape.line_width
  // Compute the strokeWidth through has_frame_color
  const lineWidth = (piechartShape.has_frame_color) ? edge : 0
  // Compute the fill color through has_fill_color
  const fillColor = (piechartShape.has_inside_color) ? piechartShape.fill_color : 'none'
  // Tooltip
  ...
  Object.defineProperty(initial, 'transformedCornerCoord', {
    get: function () {
      ...
      y2 = scale * (y2 - yc) + yc
      // Rotating
      const sinphi = Math.sin(initial.angle * (2 * Math.PI) / 360)
      const cosphi = Math.cos(initial.angle * (2 * Math.PI) / 360)
      const xoff = (x1 - xc) * cosphi - (y1 - yc) * sinphi - (x1 - xc)
      const yoff = (x1 - xc) * sinphi + (y1 - yc) * cosphi - (y1 - yc)
      // Add the offset
  ...
  Object.defineProperty(initial, 'relCoord', {
    get: function () {
      const width = initial.transformedCornerCoord.x2 - initial.transformedCornerCoord.x1
      const height = initial.transformedCornerCoord.y2 - initial.transformedCornerCoord.y1
  ...
  if (['piechart'].includes(piechartShape.shape)) {
    Object.defineProperty(initial, 'piechartPath', {
      get: function () {
        const interim = pointArrayToPiechartString(initial.relPoints, initial.startAngle, initial.endAngle, initial.edge)
        return interim
      }
    })
  }
  ...
  Object.defineProperty(initial, 'writeAccess', {
    get: function () {
      const current = ComSocket.singleton().oVisuVariables.get('.currentuserlevel')!.value
