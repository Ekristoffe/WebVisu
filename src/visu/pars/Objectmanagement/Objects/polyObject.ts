...
export function createPolyObject (polyShape: IPolyShape, dynamicElements : Map<string, string[][]>) : IPolyObject {
  ...
  const absCornerCoord = { x1: polyShape.rect[0], y1: polyShape.rect[1], x2: polyShape.rect[2], y2: polyShape.rect[3] }
  // absCenterCoord are the coordinates of the rotation and scale center
  const absCenterCoord = { x: polyShape.center[0], y: polyShape.center[1] }
  // relCoord are the width and the height in relation the div
  const relCoord = { width: polyShape.rect[2] - polyShape.rect[0], height: polyShape.rect[3] - polyShape.rect[1] }
  // the relCenterCoord are the coordinates of the midpoint of the div
  const relMidpointCoord = { x: (polyShape.rect[2] - polyShape.rect[0]) / 2, y: (polyShape.rect[3] - polyShape.rect[1]) / 2 }
  ...
  const edge = (polyShape.line_width === 0) ? 1 : polyShape.line_width
  // Compute the strokeWidth through has_frame_color
  const lineWidth = (polyShape.has_frame_color) ? edge : 0
  // Compute the fill color through has_fill_color
  const fillColor = (polyShape.has_inside_color) ? polyShape.fill_color : 'none'
  // Tooltip
  ...
  Object.defineProperty(initial, 'relPoints', {
    get: function () {
      ...
      const interim = []
      const xoff = initial.transformedCornerCoord.x1 - initial.edge
      const yoff = initial.transformedCornerCoord.y1 - initial.edge
  ...
  Object.defineProperty(initial, 'cssTransformOrigin', {
    get: function () {
      const interim = '' + (absCenterCoord.x - absCornerCoord.x1) + 'px ' + (absCenterCoord.y - absCornerCoord.y1) + 'px ' as string
      return interim
    }
  })
  ...
  Object.defineProperty(initial, 'cssTransform', {
    get: function () {
      ...
      const interim = 'scale(' + scale + ') rotate(' + initial.angle + 'deg) translate(' + initial.xpos + 'px, ' + initial.ypos + 'px)'
      return interim
    }
  })
