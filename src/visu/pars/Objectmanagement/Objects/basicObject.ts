...
export function createBasicObject (basicShape: IBasicShape, dynamicElements : Map<string, string[][]>) : IBasicObject {
  ...
  const absCornerCoord = { x1: basicShape.rect[0], y1: basicShape.rect[1], x2: basicShape.rect[2], y2: basicShape.rect[3] }
  // absCenterCoord are the coordinates of the rotation and scale center
  const absCenterCoord = { x: basicShape.center[0], y: basicShape.center[1] }
  // relCoord are the width and the height in relation the div
  const relCoord = { width: basicShape.rect[2] - basicShape.rect[0], height: basicShape.rect[3] - basicShape.rect[1] }
  // the relCenterCoord are the coordinates of the midpoint of the div
  const relMidpointCoord = { x: (basicShape.rect[2] - basicShape.rect[0]) / 2, y: (basicShape.rect[3] - basicShape.rect[1]) / 2 }
  ...
  const edge = (basicShape.line_width === 0) ? 1 : basicShape.line_width
  // Compute the strokeWidth through has_frame_color
  const lineWidth = (basicShape.has_frame_color) ? edge : 0
  // Compute the fill color through has_fill_color
  const fillColor = (basicShape.has_inside_color) ? basicShape.fill_color : 'none'
  // Tooltip
