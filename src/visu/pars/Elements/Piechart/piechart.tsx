...
export const Piechart: React.FunctionComponent<Props> = ({ section }) => {
  ...
  let piechart: IPiechartShape = {
    ...
    has_inside_color: util.stringToBoolean(
      section.getElementsByTagName("has-inside-color")[0].textContent
    ),
    fill_color: util.rgbToHexString(
      section.getElementsByTagName("fill-color")[0].textContent
    ),
    fill_color_alarm: util.rgbToHexString(
      section.getElementsByTagName("fill-color-alarm")[0].textContent
    ),
    has_frame_color: util.stringToBoolean(
      section.getElementsByTagName("has-frame-color")[0].textContent
    ),
    frame_color: util.rgbToHexString(
      section.getElementsByTagName("frame-color")[0].textContent
    ),
    frame_color_alarm: util.rgbToHexString(
      section.getElementsByTagName("frame-color-alarm")[0].textContent
    ),
    line_width: Number(
    ...
    hidden_input: util.stringToBoolean(
      section.getElementsByTagName("hidden-input")[0].textContent
    ),
    enable_text_input: util.stringToBoolean(
      section.getElementsByTagName("enable-text-input")[0].textContent
    ),
    ...
    tooltip:
      section.getElementsByTagName("tooltip").length > 0
        ...
        : "",
    access_levels: section.getElementsByTagName("access-levels").length
      ? util.parseAccessLevels(
          section.getElementsByTagName("access-levels")[0].innerHTML
        )
  ...
  if (section.getElementsByTagName("enable-text-input").length) {
    if (
      section.getElementsByTagName("enable-text-input")[0].textContent == "true"
    ) {
  ...
  return useObserver(() => (
    ...
    >
      ...
      >
        <svg
          onClick={
            onclick == null ? null : state.writeAccess ? () => onclick() : null
          }
          onMouseDown={
            onmousedown == null
              ? null
