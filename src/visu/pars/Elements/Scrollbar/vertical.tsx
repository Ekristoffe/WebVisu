...
export const VerticalScrollbar: React.FunctionComponent<Props> = ({
  ...
  const increment = () => {
    let upper =
      state.lowerBound < state.upperBound ? state.upperBound : state.lowerBound;
  ...
  const decrement = () => {
    let upper =
      state.lowerBound < state.upperBound ? state.lowerBound : state.upperBound;
    if (state.value > upper) {
      updateFunction(state.value - 1);
    }
  };
    ...
    if (selected && updateFunction !== undefined) {
      ...
      if (!(delta < 0 || delta > spacing)) {
        ...
        if (state.lowerBound > state.upperBound) {
          let nextScrollvalue =
            state.lowerBound - (1 - delta / spacing) * scrollIntervall;
