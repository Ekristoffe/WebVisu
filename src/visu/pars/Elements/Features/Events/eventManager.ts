      ...
      for (let i = 0; i < expressions.length; i++) {
        ...
        if (expr.length) {
          ...
          if (tagName === "placeholder") {
            ...
            if (
              StateManager.singleton().oState.get("USECURRENTVISU") === "TRUE"
            ) {
              clickFunction = function (): void {
      ...
      if (actionList.getElementsByTagName("expr-link").length) {
        ...
        for (let i = 0; i < links.length; i++) {
          let link = actionList
            ...
            [i].getElementsByTagName("expr")[0].children[0];
          let type = link.tagName;
          let content = link.textContent;
          if (type === "var") {
            clickFunction = function (): void {
              if (
                ComSocket.singleton().oVisuVariables.has(content.toLowerCase())
              ) {
