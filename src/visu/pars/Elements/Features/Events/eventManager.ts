...
export function parseDynamicShapeParameters (section: Element): Map<string, string[][]> {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (tags.includes(exprName)) {
      ...
      const expressions = children[i].getElementsByTagName('expr')[0].children
      // Init a helper stack
      const stack: string[][] = []
...
export function parseDynamicTextParameters (section: Element, shape: string): Map<string, string> {
  ...
  for (let i = 0; i < children.length; i++) {
    const exprName = children[i].nodeName
    if (tags.includes(exprName)) {
      const expressions = children[i].getElementsByTagName('expr')
      // The text could be dynamic with a expression reference
      for (let j = 0; j < expressions.length; j++) {
        if (expressions[j].getElementsByTagName('var')[0] !== undefined) {
          const varName = expressions[j].getElementsByTagName('var')[0].textContent.toLowerCase()
          ...
          if (expressions[j].getElementsByTagName('const')[0] !== undefined) {
            const constName = expressions[j].getElementsByTagName('const')[0].textContent.toLowerCase()
          } else {
            console.log('A variable textfield has no valid variable attached!')
          }
        }
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (exprName === 'expr-toggle-var') {
      ...
      for (let i = 0; i < expressions.length; i++) {
        ...
        const variable = expressions[i].getElementsByTagName('var')
        if (variable.length) {
      ...
      for (let i = 0; i < expressions.length; i++) {
        ...
        if (expr.length) {
          ...
          if (tagName === 'placeholder') {
            ...
            if (StateManager.singleton().oState.get('USECURRENTVISU') === 'TRUE') {
              clickFunction = function (): void {
                const visuname = ComSocket.singleton().oVisuVariables.get(visuVariable)!.value
                ComSocket.singleton().setValue('.currentvisu', visuname)
              }
            } else {
              clickFunction = function (): void {
    ...
    if (exprName === 'input-action-list') {
      ...
      if (actionList.getElementsByTagName('expr-assign').length) {
        const assigns = actionList.getElementsByTagName('expr-assign')
        for (let i = 0; i < assigns.length; i++) {
          const action = assigns[0]
          // Left side value. Must be a variable.
          const lvalue = action.getElementsByTagName('lvalue')[0].getElementsByTagName('expr')[0].getElementsByTagName('var')[0].textContent
          // Right sided expression
          const rpnStack: string[][] = []
          const rvalue = action.getElementsByTagName('rvalue')[0].getElementsByTagName('expr')[0].children
          ...
          clickFunction = function (): void {
            const rvalue = ComSocket.singleton().evalFunction(rpnStack)()
            const com = ComSocket.singleton()
      ...
      if (actionList.getElementsByTagName('execute').length) {
        ...
        const executes = actionList.getElementsByTagName('execute')
      ...
      if (actionList.getElementsByTagName('expr-link').length) {
        const links = actionList.getElementsByTagName('expr-link')
        for (let i = 0; i < links.length; i++) {
          const link = actionList.getElementsByTagName('expr-link')[i].getElementsByTagName('expr')[0].children[0]
          const type = link.tagName
          const content = link.textContent
          if (type === 'var') {
            clickFunction = function (): void {
              if (ComSocket.singleton().oVisuVariables.has(content.toLowerCase())) {
                const varContent = ComSocket.singleton().oVisuVariables.get(content.toLowerCase())!.value
                window.open(varContent.split(' ')[0])
              }
            }
          } else {
            clickFunction = function (): void {
              const value = ComSocket.singleton().evalFunction([[type, content.toLowerCase()]])()
              window.open(content)
            }
          }
          stack.push(clickFunction)
...
export function parseScrollUpdate (section : Element) : Function {
  ...
  if (updateExpr.length) {
    const content = updateExpr[0].getElementsByTagName('expr')[0].children[0]
