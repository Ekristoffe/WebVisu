...
export const Textfield: React.FunctionComponent<Props> = ({
  ...
  let hasStrikeOut = util.stringToBoolean(
    section.getElementsByTagName("font-strike-out")[0].textContent
  );
  ...
  let textAlignHorz = section.getElementsByTagName("text-align-horz")[0]
    .textContent;
  let textAlignVert = section.getElementsByTagName("text-align-vert")[0]
    .textContent;
  ...
  if (dynamicParameters.has("expr-text-flags")) {
    ...
    Object.defineProperty(initial, "textAlignHorz", {
      get: function () {
        let value = Number(
          ComSocket.singleton().oVisuVariables.get(element)!.value
        );
        if (value / 8 > 0) {
          if (value == 4) {
            return "center";
  ...
  if (dynamicParameters.has("expr-font-flags")) {
    ...
    Object.defineProperty(initial, "hasUnderline", {
      get: function () {
        let value =
          (Number(ComSocket.singleton().oVisuVariables.get(element)!.value) &
            4) >
          0
            ? true
            : false;
  ...
  if (dynamicParameters.has("expr-font-height")) {
    ...
    Object.defineProperty(initial, "fontHeight", {
      get: function () {
        let value =
          -1 * Number(ComSocket.singleton().oVisuVariables.get(element)!.value);
        value = value / 1.4;
        return value;
      },
    });
    ...
    >
      <React.Fragment>
        {svgTextLines.map((element, index) => (
          <React.Fragment key={uid(element)}>{element}</React.Fragment>
        ))}
      </React.Fragment>
    </text>
  ));
