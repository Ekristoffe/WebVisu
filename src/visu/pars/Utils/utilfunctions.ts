...
export function computeMinMaxCoord (pointArray : number[][]): number[] {
  const rect = [pointArray[0][0], pointArray[0][1], pointArray[0][0], pointArray[0][1]]
...
export function pointArrayToPiechartString (pointArray : number[][], startAngleDegree : number, endAngleDegree : number, strokeWidth: number) : string {
  ...
  const degreeToRad = 2 * Math.PI / 360
  const startAngleRad = -startAngleDegree * degreeToRad
  const endAngleRad = -endAngleDegree * degreeToRad
  // Calculate the radii of the ellipse
  const radiusx = Math.abs(pointArray[1][0] - 1 - pointArray[0][0])
  const radiusy = Math.abs(pointArray[1][1] - 1 - pointArray[0][1])
  ...
  const start = [Math.cos(startAngleRad) * rStart + radiusx, Math.sin(startAngleRad) * rStart + radiusy]
  const end = [Math.cos(endAngleRad) * rEnd + radiusx, Math.sin(endAngleRad) * rEnd + radiusy]
  const interimArray = pointArray
  interimArray[2] = start
  interimArray[3] = end

  // Claculate the largeArcFlag
  const angleDiff = endAngleDegree - startAngleDegree
  let largeArcFlag = 1
  if (angleDiff > 0) {
