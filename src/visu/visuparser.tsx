...
export const Visualisation: React.FunctionComponent<Props> = React.memo(
  ({ visuname, mainVisu, replacementSet, width }) => {
    ...
    React.useEffect(() => {
      let fetchXML = async function () {
        ...
        if ((await get(visuname)) === undefined) {
          ...
          if (xml == null) {
            console.log(
              "The requested visualisation " + visuname + " is not available!"
            );
          } else {
        ...
        if (plainxml !== null) {
          ...
          setOriginSize(
            stringToArray(
              xmlDoc
                .getElementsByTagName("visualisation")[0]
                .getElementsByTagName("size")[0].innerHTML
            )
          );
          setLoading(false);
        }
